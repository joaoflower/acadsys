			...
			<table border="0" cellpadding="0" cellspacing="0" class="tlistdata">
			  ...
			  <?
				...
				$tNota = "nota".$_SESSION['sFrameCod_car'];
			  	...
			  	$vQuery = "select cmt.*, no.can_not, no.nota1, no.nota2, no.nota3, no.not_cur ";
				$vQuery .= "from ( ";
				...
				//$vQuery .= "and no.can_not <= $vNro_not";
			  ...
			  <tr <?=ftrstyle($vCont)?>  onMouseOver="mouseover(this)" onMouseOut="mouseout(this)" id="rTr<?=$aResult['cod_cur']?>">
	            ...
	            <td <?=ftdstyle($vNum_rows, $vCont)?>><input name="<?=strlen($aResult['num_mat']).$aResult['num_mat']?>" type="text" class="<?=(fVerinotaapr($aResult['nota'.$vNro_not])?"notaapr":"notades")?>" id="<?=strlen($aResult['num_mat']).$aResult['num_mat']?>" value="<?=$aResult['nota'.$vNro_not]?>" size="2" maxlength="2" onkeyup="fverinota(this)"><input name="rMod_not<?=strlen($aResult['num_mat']).$aResult['num_mat']?>" type="hidden" id="rMod_not<?=strlen($aResult['num_mat']).$aResult['num_mat']?>" value="<?=$aResult['mod_not'].$aResult['mod_mat'].(fIsnota($aResult['not_cur'])?"u":"i").$vNro_not?>" /></td>
			  </tr>
			  <?
					$vCont++;
			  	}
