	...
	if(fsafetysetcaranoper())
	{
		...
		{			
			...
			if($vCan_not > 0)
			{
				...
				{
					$vSize = substr($_POST['rNum_mat'], $vLenght, $vDigit);
					$vLenght += $vDigit;
										
					$vNum_mat = substr($_POST['rNum_mat'], $vLenght, $vSize);
					$vMod_not = substr($_POST['rMod_not'], 6 * $ii, 2);
					$vMod_mat = substr($_POST['rMod_not'], (6 * $ii) + 2, 2);
					$vIns_Upd = substr($_POST['rMod_not'], (6 * $ii) + 4, 1);
					$vNro_not = substr($_POST['rMod_not'], (6 * $ii) + 5, 1);
					...
					if($vIns_Upd == "i")
					{
						...
						$vQuery .= "can_not, nota$vNro_not, not_cur, cod_act, fch_not, cod_usu, obs_not) values ";
						...
						$vQuery .= "'$vMod_not', '$vMod_mat', '3', '$vNot_cur', '$vNot_cur', '{$_SESSION['sActaCod_act']}', now(), ";
					...
					elseif($vIns_Upd == "u")
					{	
						$vQuery = "update $tNota set nota$vNro_not = '$vNot_cur', ";
						$vQuery .= "not_cur = round((ifnull(nota1, 0) + ifnull(nota2, 0) + ifnull(nota3, 0)) / (if(ifnull(nota1, -1) >= 0, 1, 0) + if(ifnull(nota2, -1) >= 0, 1, 0) + if(ifnull(nota3, -1) >= 0, 1, 0) ))";
