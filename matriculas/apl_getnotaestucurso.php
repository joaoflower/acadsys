		...
		<td align="center">
			<table border="0" cellpadding="0" cellspacing="0" class="tlistdata">
			  ...
			  <tr <?=ftrstyle($vCont)?>  onMouseOver="mouseover(this)" onMouseOut="mouseout(this)" id="rTr<?=$aResult['cod_cur']?>">
	            ...
	            <td <?=ftdstyle($vNum_rows, $vCont)?>><input name="<?=$aResult['num_mat']?>" type="text" class="<?=(fVerinotaapr($aResult['not_cur'])?"notaapr":"notades")?>" id="<?=$aResult['num_mat']?>" value="<?=$aResult['not_cur']?>" size="2" maxlength="2" onkeyup="fverinota(this)"><input name="rMod_not<?=$aResult['num_mat']?>" type="hidden" id="rMod_not<?=$aResult['num_mat']?>" value="<?=$aResult['mod_not'].$aResult['mod_mat'].(fIsnota($aResult['not_cur'])?"u":"i")?>" /></td>
			  </tr>
			  <?
					$vCont++;
			  	}
